...
# CyberPanel End-to-End Provisioning Test
# Usage: bash E2E_TEST_CYBERPANEL.sh
...
CYBERPANEL_HOST="${CYBERPANEL_HOST:-localhost}"
CYBERPANEL_PORT="${CYBERPANEL_PORT:-8090}"
CYBERPANEL_API_TOKEN="${CYBERPANEL_API_TOKEN:-your_api_token_here}"
...
echo ""
echo "╔════════════════════════════════════════════════════════════╗"
echo "║  CyberPanel End-to-End Provisioning Test                   ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
...
# Test 2: Save CyberPanel credentials
log_step "Test 2: Save CyberPanel credentials to database"
SAVE_CYBER=$(api_call PUT "/api/admin/settings/cyberpanel" "{
  \"host\": \"$CYBERPANEL_HOST\",
  \"port\": $CYBERPANEL_PORT,
  \"apiToken\": \"$CYBERPANEL_API_TOKEN\",
  \"useHTTP\": true
}")

if echo "$SAVE_CYBER" | grep -q "success"; then
  log_success "CyberPanel credentials saved to database"
else
  log_error "Failed to save CyberPanel credentials"
  echo "$SAVE_CYBER"
  exit 1
fi

# Test 3: Test CyberPanel connection
log_step "Test 3: Test connection to CyberPanel"
TEST_CONN=$(api_call POST "/api/admin/settings/cyberpanel/test" "")
...
# Test 13: Verify in CyberPanel
log_step "Test 13: Verify account exists in CyberPanel"
log_success "Account provisioning complete!"
...
echo ""
echo "Next steps:"
echo "  1. Login to CyberPanel to verify account creation"
